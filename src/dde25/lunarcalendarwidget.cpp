...
private:
    mutable QMap<QDate, CaLunarDayInfo> m_caHuangLiDayMap;
};
...
QString CalenderStyle::getLunarDayName(const QDate &date) const
{
    ...
    CaLunarDayInfo &info = m_caHuangLiDayMap[date];
...
void CalenderStyle::updateLunarInfo(const QDate &date) const
{
    ...
    if (!m_caHuangLiDayMap.contains(date)) {
        // 本项目的农历数据按月缓存，先确保该月已就绪（已加载时只是一次哈希查询）。
        DDE25::LunarCache::instance()->ensureMonth(date.year(), date.month());
        m_caHuangLiDayMap[date] = DDE25::LunarCache::instance()->info(date);
    }
}
        ...
        if (w && w->objectName() == "qt_calendar_calendarview") {
            if (const QStyleOptionViewItem *vopt = qstyleoption_cast<const QStyleOptionViewItem *>(opt)) {
                ...
                if (!(vopt->state & QStyle::State_Enabled)) {
                    p->save();
                    p->setPen(Qt::NoPen);
                    p->setBrush(getColor(vopt, QPalette::Window));
                ...
                if (vopt->index.row() == 0) {
                    p->save();
                    QColor color = getColor(vopt, QPalette::Mid);
                    color.setAlpha(static_cast<int>(255 * 0.05));
                    QPen pen(color, 2);
                    p->setPen(pen);
                    int margins = pixelMetric(QStyle::PM_LayoutLeftMargin, opt, w);
    ...
    : QCalendarWidget(parent)
{
    ...
    label->setObjectName("BottomSpacingLabel");
