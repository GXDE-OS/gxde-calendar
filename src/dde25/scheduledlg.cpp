 ...
 * DTK2Widget-Qt6.
 * ----------------------------------------------------------------------------
 * 移植自 dde-calendar（src/calendar-client/src/dialog/scheduledlg.cpp）。
 * 差异见头文件注释。
 */
    ...
    , m_createAllDay(isAllDay)
{
    ...
    initConnection();
    setTabFouseOrder();
    //参考实现这里是 slotAccountUpdate()：填帐户下拉框后初始化类型下拉框。
    //本项目没有帐户，剩下的就是这个初始化
    initJobTypeComboBox();
...
bool CScheduleDlg::selectScheduleType()
{
    ...
    if (m_typeComBox->isEditable()) {
        ...
        //参考实现是让帐户异步建类型再回调，这里进程内同步建，
        //返回值就是新类型的 ID；建失败就直接不放行
        const QString typeID = CalendarService::instance()->createScheduleType(type);
        if (typeID.isEmpty()) {
            return false;
        }
        return createSchedule(typeID);
...
bool CScheduleDlg::createSchedule(const QString &scheduleTypeId)
{
    ...
    if (beginDateTime > endDateTime) {
        ...
        //参考实现用的是 DTK 内置警告图标，本机没装那套图标主题，取系统主题里的
        ...
        prompt->exec();
        //参考实现漏了 delete，这里补上，避免每弹一次泄漏一个对话框
        delete prompt;
        return false;
    }
    ...
    // 重复结束规则只在真的选了重复方式（「重复」下拉不是「从不」）时才有意义，
    // 这里套一层判断。参考实现是无条件调的，但 KCalendarCore 的
    // Recurrence::setDuration()/setEndDateTime() 内部都走 defaultRRule(true)：
    // 当前没有 RRULE 时会凭空补一条没有任何频率的空规则，序列化出来就是 libical
    // 的错误文本 "RRULE:ERROR: No Value"。结果是「不重复」的日程被存成带脏 RRULE
    // 的日程，重开时 recurs() 还会误报 true（显示端靠 hasUsableRecurrence() 才没
    // 出错），存库和导出的 ICS 也都是脏的。加上判断后语义不变：「从不」本来就不该
    // 造出重复规则，而「永不」要的 duration=-1 正是 setRRuleType() 新建的
    // RecurrenceRule 的默认值。
    if (m_beginrepeatCombox->currentIndex() > 0) {
    ...
    //参考实现是 CScheduleOperation（DBus 到 service 进程），这里是进程内直调
    bool res = false;
    if (m_type == 1) {
        //创建日程
        schedule->setUid("0");
        res = !CalendarService::instance()->createSchedule(schedule).isEmpty();
        ...
        res = CalendarService::instance()->updateSchedule(schedule);
    }
    return res;
}
...
{
    if (m_typeComBox->lineEdit() != nullptr && m_typeComBox->lineEdit()->text().isEmpty()) {
        //名称为空
...
void CScheduleDlg::initUI()
{
    ...
    //全天
    {
        ...
        m_allDayCheckbox = new QCheckBox(this);
    ...
    //时间
    {
        ...
        //参考实现用 DLabel::setElideMode，dtk2 的 DLabel 没有这个接口，去掉
    ...
    //重复
    {
        ...
        m_beginrepeatCombox = new QComboBox(this);
        //设置对象名称和辅助显示名称
    ...
    //结束重复
    {
        ...
        m_endrepeatCombox = new QComboBox(this);
        //设置对象名称和辅助显示名称
        ...
        //DTK2Widget 的 DLineEdit 直接继承 QLineEdit，没有 lineEdit()，校验器挂在它自己身上
        ...
        m_endrepeattimesWidget = new QWidget;
        //设置对象名称和辅助显示名称
...
void CScheduleDlg::initConnection()
{
    ...
    connect(m_textEdit, &QTextEdit::textChanged, this, &CScheduleDlg::slotTextChange);
    connect(m_endrepeattimes, &QLineEdit::textChanged, this, &CScheduleDlg::slotendrepeatTextchange);

    //Qt6 里 QCheckBox::stateChanged(int) 已废弃，用 checkStateChanged 转一下
    connect(m_allDayCheckbox, &QCheckBox::checkStateChanged, this,
            [this](Qt::CheckState state) {
                slotallDayStateChanged(state == Qt::Checked ? 1 : 0);
            });
    ...
    connect(m_beginDateEdit, &QDateEdit::userDateChanged, this, &CScheduleDlg::slotBDateEidtInfo);
    QShortcut *shortcut = new QShortcut(this);
    shortcut->setKey(QKeySequence(QLatin1String("ESC")));
    connect(shortcut, &QShortcut::activated, this, &CScheduleDlg::close);
    ...
    connect(m_typeComBox, &QComboBox::editTextChanged, this, &CScheduleDlg::slotTypeEditTextChanged);
...
void CScheduleDlg::initRmindRpeatUI()
{
    ...
    // 先看有没有 RRULE：没有规则时 defaultRRuleConst() 返回空，Recurrence::duration()
    // 给的是 0，会掉进下面的「结束于日期」分支，把从来没设过的结束日期填进
    // m_endRepeatDate。「从不」的日程本来就不写 RRULE（见 createSchedule 里的判断），
    // 所以没有规则一律按「永不」回填。
    if (!m_scheduleDataInfo->recurs() || m_scheduleDataInfo->recurrence()->duration() < 0) {
        //永不
        m_endrepeatCombox->setCurrentIndex(0);
...
void CScheduleDlg::setTheMe(const int type)
{
    ...
    QPalette pa = m_textEdit->palette();
    //设置颜色
    pa.setColor(QPalette::Text, titleColor);
...
/**
 * @brief CScheduleDlg::resetColor  复位颜色选择器并恢复上一次的选择
 *
 * 参考实现读的是 CConfigSettings，这里用 QSettings；键名与
 * ColorSeletorWidget 写入时保持一致（LastSysColorTypeNo 存的是颜色 ID）。
 */
void CScheduleDlg::resetColor()
{
    m_colorSeletorWideget->resetColorButton();
    QSettings settings;
    //将用户上一次选择的自定义颜色添加进去
    QString colorName = settings.value("LastUserColor", "").toString();
    ...
    //选中上一次选中的颜色
    QVariant colorId = settings.value("LastSysColorTypeNo", -1);
    int colorNum = 0;
    if (colorId.typeId() == QMetaType::Int) {
        //如果是int型表示为旧颜色编号
        colorNum = colorId.toInt();
    } else {
        const QString colorIdStr = colorId.toString();
        //如果都为空表示为初始状态，则选中第一个
        if (colorName.isEmpty() && colorIdStr.isEmpty()) {
            colorNum = -1;
        } else if (!colorIdStr.isEmpty()) {
            //如果颜色id不为空则表示颜色为内置颜色。
            //参考实现取的是 GTypeColor.keys().indexOf(colorIdStr)，
            //这里的内置色来自数据层的系统调色板，按同样的顺序找同 ID 的颜色下标
            colorNum = -1;
            int index = 0;
            const DTypeColor::List colorList = CalendarService::instance()->getSysColors();
            for (const DTypeColor::Ptr &var : colorList) {
                if (var->colorID() == colorIdStr) {
                    colorNum = index;
                    break;
                }
                ++index;
...
void CScheduleDlg::setOkBtnEnabled()
{
    ...
    if (m_typeComBox->lineEdit() != nullptr) {
